<?php  if (!defined("IS_INITPHP")) exit("Access Denied!");  /* INITPHP Version 1.0 ,Create on 2014-01-08 14:58:53, compiled from template/adminweb/sys_setting.tpl */ ?>
...
<form id='commitform' action="<?php echo $commitaction ?>" method="POST" target="hidden_frame">
...
<ul  class="ulstyle">
	<li>双击标签：<label><input type="radio" name="dbclick_tag" value="close_tag" class="input_mid" <?php if (empty($set_arr['dbclick_tag']) || $set_arr['dbclick_tag'] == 'close_tag') { ?>checked<?php } ?> >关闭标签</label>
    <label><input type="radio" name="dbclick_tag" value="newopen_tag"  class="input_mid" <?php if ($set_arr['dbclick_tag'] == 'newopen_tag') { ?>checked<?php } ?> >在新窗口打开</label>
    <label><input type="radio" name="dbclick_tag" value="return_tag" class="input_mid" <?php if ($set_arr['dbclick_tag'] == 'return_tag') { ?>checked<?php } ?> >返回菜单首页</label></li>
</ul>
...
<div style=" display:none" class="body_bottom">
    ...
    <div>
        <ul class='ulstyle'>
            <li><?php echo $whouse ?></li>
        </ul>
    </div>
	
    <!--设置2-->    
    <div style="margin-top:20px; line-height:20px;">
    	<div>销售下单需要检测库存的发货仓库：<?php echo $check_stock_whouse_html ?></div>
    ...
    <!--设置3-->
    <div style="margin-top:20px; line-height:20px;">
    	<div>销售下单的匹配订单直接出库中禁止匹配的的仓库：<?php echo $check_fbden_whouse_html ?></div>
...
<div style=" display:none" class="body_bottom">
	...
	<!--产品列表成品币种-->
    <div style="margin-top:20px; line-height:20px;">
    	<div>产品列表成本价所属币别：<?php echo $product_list_cost_html ?></div>
        <div  style="margin-left:145px"><span class="tips">勾选了币别，需要在汇率调整添加该币别的汇率，否则数据可能出错</span></div>
    </div>

    <!--销售下单时估算运费-->
    <div style="margin-top:20px; line-height:20px;">
    	<div>销售下单时估算运费： &nbsp; &nbsp; &nbsp; &nbsp;<?php echo $selling_countship_html ?></div>
    </div>
    
    <!--系统本位币-->
    <div style="margin-top:20px; line-height:20px;">
    	<div>系统本位币： &nbsp; &nbsp; &nbsp; &nbsp;  &nbsp;  &nbsp;  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;<?php echo $system_defaultcoin_html ?></div>
    </div>
...
<div style="height:180px; display:none" class="body_bottom">
	<table  cellpadding="3" cellspacing="0" id="commomform">
		<tr>
			<td>当前Logo：</td>
			<td><img src="<?php echo $showurl ?>" style="width:<?php echo $imgwidth ?>px; height:<?php echo $imgheight ?>px;"/></td>
            <TD width="250">&nbsp;</TD>
		</tr>
		<tr>
			<td>上传Logo：</td>
			<td>
			<form id="uploadform" action="<?php echo $uploadaction ?>" method="POST" enctype="multipart/form-data" target="hidden_frame">
