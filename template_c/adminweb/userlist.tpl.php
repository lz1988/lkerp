<?php  if (!defined("IS_INITPHP")) exit("Access Denied!");  /* INITPHP Version 1.0 ,Create on 2014-01-10 11:12:33, compiled from template/adminweb/userlist.tpl */ ?>
...
名字：<input type="text" name="username" id="sku" value="<?php echo $username ?>" title="输入中文名或英文名"/>&nbsp; &nbsp; &nbsp;
部门名称：<?php echo $shou_datagroup_html ?>&nbsp; &nbsp; &nbsp;
部门属性：<?php echo $backdatacatg_html ?>
...
<table id="mytable" cellspacing="0"  width="1100">
  ...
  <tr  <?php echo ($r['status'] == '已关闭')?'class="grey"':'' ?> >
    <td><a href="<?php echo $r['picurl'] ?>" target="_blank" class="tooltip"><?php echo $r['chi_name'] ?></a>&nbsp;</td>
    <td><?php echo $r['eng_name'] ?>&nbsp;</td>
    <td><?php echo $r['telphone'] ?>&nbsp;</td>
    <td><?php echo $r['email'] ?>&nbsp;</td>
    <td><?php echo $r['msn'] ?>&nbsp;</td>
    <td><?php echo $r['role'] ?>&nbsp;</td>
    <td><?php echo $r['groupname'] ?>&nbsp;</td>
    <td><?php echo $r['catg_name'] ?>&nbsp;</td>
    <td><?php echo $r['status'] ?>&nbsp;</td>    
    <td><a href="index.php?action=user_list&amp;detail=edit&amp;uid=<?php echo $r['uid'] ?>" title='修改'><img src="./staticment/images/editbody.gif" border="0"></a>&nbsp;<a href="javascript:void(0);delitem(<?php echo $r['uid'] ?>,'del')" title='删除'><img src="./staticment/images/deletebody.gif" border="0"></a>&nbsp;
      <?php if ($r['isuse']==1) { ?>
        <a href="javascript:void(0);delitem(<?php echo $r['uid'] ?>,'cls')" title='关闭此帐号'>
          <img src="./staticment/images/sendfailed.gif" border="0" /></a>
      <?php } else { ?>
          <a href="javascript:void(0);delitem(<?php echo $r['uid'] ?>,'enable')" title='启用此帐号'>
...
<input type="hidden" name="uid" value="<?php echo $uid ?>" />
  <table  border="1" cellpadding="3" cellspacing="0"  width="630px" id="commomform">
    <tr> 
      ...
      <td><input type="text" id="username" name="username" value="<?php echo $data['username'] ?>" class="check_notnull" />*</td>
      <td>&nbsp;<span class="tips">登陆用的帐号</span><span style="background:#f8f8f8;font-size:12px;color:#f00"></span></td>
    </tr> 
  <tr>
    <td align="right">密码：</td>
    <td><input type="password" name="password" disabled="disabled" value="<?php echo $data['password'] ?>" class="check_notnull" />*</td>
    <td>&nbsp;<span style="background:#f8f8f8;font-size:12px;color:#f00"></span><input type="button" value="重置密码" id="subinput" onClick="resetpass(<?php echo $uid ?>)"></td>
  </tr>
  <tr>
    <td align="right">英文名：</td>
    <td><input type="text" name="eng_name"  value="<?php echo $data['eng_name'] ?>"/></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td align="right">中文名：</td>
    <td><input type="text" name="chi_name" value="<?php echo $data['chi_name'] ?>" class="check_notnull"  />*</td>
    <td>&nbsp;<span style="background:#f8f8f8;font-size:12px;color:#f00"></span></td>
  </tr>
    <tr>
    <td align="right">电话：</td>
    <td><input type="text" name="telphone" value="<?php echo $data['telphone'] ?>"/></td>
    <td>&nbsp;</td>
  </tr>
    <tr>
  ...
  <tr>
    ...
    <?php foreach ($datalist as $key=>$r) { ?>
    <option value="<?php echo $r[id] ?>" <?php if($r['id']==$data['groupid'])echo 'selected' ?> ><?php echo $r[groupname] ?></option>
    <?php } ?>
    </select>*</td>
    <td>&nbsp;<span style="background:#f8f8f8;font-size:12px;color:#f00"></span></td>
  </tr>
  <tr>
    ...
    <?php foreach ($roleArr as $key=>$r) { ?>
    <option value="<?php echo $key ?>" <?php if($key==$data['roleid'])echo 'selected' ?> ><?php echo $r ?></option>
    <?php } ?>
    </select>*</td>
    <td>&nbsp;<span style="background:#f8f8f8;font-size:12px;color:#f00"></span></td>
  </tr>  
  <tr>
  	...
  	<td align="center"><span id="showpic">
        <img src="<?php echo $data['picurl'] ?>" border=0 width=100 height=100></span>
...
<form action="index.php?action=user_list&detail=newmod" method="post">
  ...
  <tr>
    ...
    <option value="">==选择部门==</option>    
    <?php foreach ($datalist as $key=>$r) { ?>
    <option value="<?php echo $r[id] ?>" ><?php echo $r[groupname] ?></option>
    <?php } ?>
    </select>*</td>
    <td>&nbsp;<span style="background:#f8f8f8;font-size:12px;color:#f00"></span></td>
  </tr>
...
$(document).ready(function(){
    ...
    new AjaxUpload(button,{
        action: 'index.php?action=user_list&detail=uploadfile&uid=<?php echo $uid ?>',
        /*data:{
