...
if ($detail == 'list') {
	...
	if ($sqlstr || $order) {
		...
		$bannerstrarr[] = array('url'=>'index.php?action=report_sold_bysuppliersku&detail=output','value'=>'导出数据');
		$asc_img 	= 'both_asc';
		$desc_img 	= 'both_desc';
        ...
        $get_user = $user->D->get_one_by_field(array('uid'=>$_SESSION['uid']),'chi_name');
        $sqlstr .= ' and esse.name="'.$get_user['chi_name'].'"';
		$datalist = $process->select_skusupplier_sold_list($sqlstr, $orders);
		...
		for ($i = 0; $i < $length; $i++) {
			...
			// echo $datalist[$i]['returnprice']."<br/>";
            // echo $datalist[$i]['totalprice']." <br/>";
    ...
    $this->V->mark(array('title'=>'供应商SKU销售报表'));
	$temp = 'pub_list';
}
...
/**
 * @title 产品表切割供应商字符串，计划执行任务 
 * @author Jerry
 * @create on 2013-12-31
 */ 
elseif ($detail == 'split_prc_esse'){
   
    $prc_esse   = $this->S->dao('prc_esse');
    $product    = $this->S->dao('product');
    $data       = $product->D->get_allstr(' and manufacturer<>""','','pid desc','pid,manufacturer');
    foreach($data as $v){
        $str = str_replace('@','',$v['manufacturer']);
        ...
        if (count($arr) > 1)
        {
            foreach($arr as $val){
                $d['pid'] = $v['pid'];
                $d['eid'] = $val;
                $ret = $prc_esse->D->get_one_by_field($d,'id');
                if (!$ret){
                    $r = $prc_esse->D->insert($d);
                    if (!$r){$error++;}
                }
            }
        }else{
    ...
    if (!$error){exit("更新成功！");}else{exit("更新失败！");}
}
