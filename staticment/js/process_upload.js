...
function challs_flash_update(){ //Flash 初始化函数
	...
	if(window.ActiveXObject){
		a.url="./staticment/dynamic/process_uploadmod.php";
	}else{
		a.url="../dynamic/process_uploadmod.php";		
	}
	...
	//engname = challs_flash_FormData("engname");
	
	a.FormID=['type','engname'];
...
function challs_flash_style(){ //组件颜色样式设置函数
	...
	a.backgroundInsideLineColor=['#A9BACC','#A9BACC'];	//组件内框线颜色，2个颜色之间过度
...
function delImage(file_path){
		$.ajax({
		type:'POST',
		url:'update_stock.php',
		data:{'action':'delImage','file_path':file_path}
	});
}