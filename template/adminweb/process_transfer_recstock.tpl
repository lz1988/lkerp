...
<!--填写资料部分-->
<form method="POST" action="index.php?action=process_recstock&detail=<!--{echo $moddetail}-->" >
...
<td><input type="hidden"  name="detail_id[]" value="<!--{echo $val['id']}-->" /><span id="row_<!--{echo $floor}-->" class="big"><!--{echo $floor}--></span></td>
        ...
        <td><input type="text"  name="quantity[]" style="width:50" value="<!--{echo $val['quantity']}-->" disabled></td><!--入库数量-->
        <td><input type="text"  name="comment[]" style="width:160px" value="<!--{echo $val['comment']}-->"  title="<!--{echo $val['comment']}-->" disabled></td><!--备注-->
