...
<input type="hidden" name="detail" value="<!--{echo $moddetail}-->" />
<input type="hidden" name="suppliername" value="<!--{echo $supplier}-->">
<input type="hidden" name="order_id" value="<!--{echo $order_id}-->" />
<!--头部表格-->
<table width="" cellpadding="3" cellspacing="0" class="commomform" border="0">
  <tr>
    <td width="">
    	<font size=-1>供应商：<input type='text'  name="supplier"  value="<!--{echo $supplier}-->" id="supplier" disabled="disabled"  title="<!--{echo $supplier}--> (供应商由备货人选择，如需更改请联系备货人)" />&nbsp; &nbsp; 
        <span style="margin-left:200px;">币别：<!--{echo $currencyhtml;}--></span>
		<span style="margin-left:30px;">运费：<input type="text" title="" id="shipping" name="shipping" value="<!--{echo $shipping}-->"/></span>
        </font>
    </td>
  </tr>
</table>
...
<!--{if ($datalistt)}-->
<!--{$floor=0}-->
<!--{foreach ($datalistt as $val)}-->
<!--{$floor++}-->
	<tr>
        <td><span id="row_<!--{echo $floor}-->" class="big"><!--{echo $floor}--></span>
            <span title=删除 class=point onclick=delrow('<!--{echo $floor}-->')>
            <img src="./staticment/images/deletebody.gif" border="0">
            <input type="hidden" name="detail_id[]" value="<!--{echo $val['detail_id']}-->" /><input type="hidden" name="id[]" value="<!--{echo $val['id']}-->" /></td>
        <td><input type="text"  name="" style="width:100px" value="<!--{echo $val['sku']}-->"  disabled></td><!--产品SKU-->
        <td><input type="text"  name="" style="width:200px" value="<!--{echo $val['product_name']}-->"  title="<!--{echo $val['product_name']}-->" disabled></td><!--产品名称-->
        <td><!--{echo $val['sourcehtml']}--></td><!--入库仓库-->
        <td><input type="text"  name="quantity[]" style="width:50px"  value="<!--{echo $val['quantity']}-->"  onblur=countsum("<!--{echo $floor}-->") id="quantity_<!--{echo $floor}-->" ></td><!--数量-->
        <td><input type="text" style="width:50px"  onblur=countsum("<!--{echo $floor}-->")  value="<!--{echo $val['price']}-->" id="price_<!--{echo $floor}-->"  title="单价并不保存用户填写，只是方便计算合计！" /></td>
        <td><input type="text"  name="e_sprice[]"  value="<!--{echo $val['e_sprice']}-->" style="width:100px" id="e_sprice_<!--{echo $floor}-->" /></td><!--不含税合计-->
        <td><input type="text"  name="e_siprice[]" style="width:100px"  value="<!--{echo $val['e_siprice']}-->" id="e_siprice_<!--{echo $floor}-->" /></td><!--价税合计-->
        <td><input type="text"  name="e_recdate[]"  value="<!--{echo $val['e_recdate']}-->" style="width:100px"  onClick="WdatePicker()"  class="Wdate" /></td><!--交货日期-->
        <td><input type="text"  name="comment[]"  value="<!--{echo $val['comment']}-->" style="width:200px"/></td><!--备注-->
   </tr>
<!--{/foreach}-->
<!--{/if}-->
</table>
