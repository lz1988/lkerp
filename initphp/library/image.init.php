...
class imageInit {
	...
	public function make_thumb($source, $newname, $width = 100, $height = 100, $isauto = true) {
		if (!file_exists($source)) {echo 'no files'; return false;}
		...
		if ($isauto == true) {
			if ($image_info['width'] < $image_info['height']) {
				$width = $width*($image_info['width']/$image_info['height']);
			} else {
				$height = $height*($image_info['height']/$image_info['width']);
			}
			/* 画布和原始图创建 */
     ...
     * @param type $newname 要生成的目录
     * @return boolean true
     * @author color
     */ 
	function copyimg ($source,$newname){
        $source_info = $this->get_img_info($source); //获取原始图片信息
        if ($source_info['type'] == 'bmp') return false;
        if ($source_info['type']) {
        ...
        $source = $temp; 
        $this->make($source_info['type'], $source, $newname, 100);
		imagedestroy($thumb);  
		return true;
    }
	...
	public function water_mark($source_path, $water_path, $postion, $pct=100, $quality=100) {
		if (!file_exists($source_path)) {
			echo 'no source path';return false;
		}
		if(!file_exists($water_path)) {
			echo 'no water_path';return false;
		}
		/* 图片信息 */
		$source_info = $this->get_img_info($source_path);
		$water_info  = $this->get_img_info($water_path);
		if ($source_info['type'] == 'bmp' || $water_info['type'] == 'bmp') { echo "bmo 图片不支持";return false;}
		if ($source_info['type']) {
			$imagecreatefrom = 'imagecreatefrom' .$source_info['type']; 
		...
		$shource_dir = dirname($source_path).'/tmp'.$_SESSION['eng_name']; //修改水印BUG
